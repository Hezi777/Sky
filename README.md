...
The point of Sky is not the dashboard, it is that **one backend contract serves
three completely different clients**.

```
native/ (SwiftUI)  ─┐
                    ├─▶ backend/ app/api/*  ─▶  GitHub, Notion, Spotify,
legacy-web/ (frozen)─┘        └── lib/*          IBKR, Groq, TASE, Finnhub
                    contract = backend/lib/types.ts
```

- **A single egress point.** Only `backend/` talks to external services. Every
  client calls `localhost:3000/api/*`. One place holds credentials, one place to
  rate-limit, one place to change when an upstream API breaks.
- **`backend/lib/types.ts` is the single source of truth** for every response
  shape. The Swift models in `native/Sky/Models/` are hand-mirrored against it, so
  a response-shape change is a deliberate synchronized edit across two languages
  rather than something that silently drifts.
- **The backend is genuinely headless.** `app/page.tsx` and `app/layout.tsx` are
  deliberate stubs that exist only so `next build` and `next dev` work. There is
  no web UI in it at all. Next.js used purely as an API layer.
- **Two documented exceptions to the rule.** The native client hits Open-Meteo and
  ZenQuotes directly, because they need no key and no secret, so routing them
  through the backend would add a hop for nothing. Exceptions written down beat
  exceptions discovered later.
- **Three independent roots.** `backend/` and `native/` each build standalone from
  their own directory. The Swift project is generated by XcodeGen from
  `native/project.yml`, so the `.xcodeproj` is never the source of truth.

No database and no cache: everything is live-fetched, single-user, no auth. A
constraint picked deliberately. It removes an entire category of staleness bugs
at the cost of being useless offline.
